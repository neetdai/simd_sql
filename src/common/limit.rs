use crate::{ParserError, common::expr::Expr, token::{TokenKind, TokenTable}};
...
impl Limit {
    pub fn new(token_table: &TokenTable, cursor: &mut usize) -> Result<Self, ParserError> {
        let first = token_table.get_kind(*cursor)
            .map(|kind| kind == &TokenKind::Identifier)
            .unwrap_or(false);
        let comma = token_table.get_kind(*cursor + 1)
            .map(|kind| kind == &TokenKind::Comma)
            .unwrap_or(false);
        let second = token_table.get_kind(*cursor + 2)
            .map(|kind| kind == &TokenKind::Identifier)
            .unwrap_or(false);
